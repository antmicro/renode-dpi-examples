...
module sim;
  ...
  initial begin
    ...
    // Starts background connection thread early
    runtime.connect_plus_args_in_background();
  end

  bit [1:0] is_connected;
  always @(posedge clk) begin
    bit did_receive;

    // Runs a connection thread in the background if not-exists
    runtime.connect_plus_args_in_background();

    is_connected = {is_connected[0], runtime.is_connected()};
    if (is_connected == 2'b01) begin
        $display("Renode connected at %t", $realtime);
    end

    // Handles message if Renode is connected and sent a message
    renode.try_receive_and_handle_message(did_receive);


    if (runtime.has_lost_connection()) $finish;
  end
